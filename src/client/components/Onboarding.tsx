...
const STORAGE_KEY = "investmade:onboarding:v2";
...
const CADENCE_OPTIONS = [
  { id: "daily", title: "1 day", description: "Build a new investment session every day." },
  { id: "weekly", title: "1 week", description: "Build a new investment session every week." },
  { id: "monthly", title: "1 month", description: "Build a new investment session every month." }
] as const;

const TICKET_OPTIONS: Array<{ id: TicketChoice; title: string; description: string }> = [
...
const RISK_OPTIONS: Array<{
...
}> = [
  {
    id: "conservative",
    title: "Conservative",
    description: "Prefer steadier signals and lower-impact routes. Value can still fall."
  },
  {
    id: "balanced",
    title: "Balanced",
  ...
  },
  {
    id: "degen",
    title: "Degen",
    description: "Accept more volatility in the ranking. This is not a promise of higher returns."
  }
];
...
const ASSET_OPTIONS: Array<{
...
}> = [
  {
    id: "BOTH",
    title: "A mix of both",
    description: "Let the private ranking compare eligible crypto and stock tokens.",
    tag: "Recommended"
  },
  {
    id: "CRYPTO",
  ...
  },
  {
    ...
    description: "Show eligible stock tokens when jurisdiction and market checks pass."
  }
];
...
export function Onboarding({
  ...
  const [draft, setDraft] = useState<PreferenceDraft>(() =>
    storedPreferences
      ? {
          cadence: storedPreferences.cadence,
          ticketSizeUsd: storedPreferences.ticketSizeUsd,
          ticketChoice: isPresetTicket(storedPreferences.ticketSizeUsd)
            ? storedPreferences.ticketSizeUsd
            : "custom",
          customTicketInput: isPresetTicket(storedPreferences.ticketSizeUsd)
            ? ""
            : String(storedPreferences.ticketSizeUsd),
          riskMode: storedPreferences.riskMode,
          assetChoice: assetChoiceFrom(storedPreferences.assetClasses),
  ...
  return (
    <main className="onboarding-page">
      <section className="onboarding-copy">
        ...
        <p>
          Answer five short questions. investmade.fun uses them only to constrain and rank
          executable candidates. Your 100 USDG stays in your wallet until you review and sign.
        </p>
        <div className="onboarding-points">
          <p><span>1</span><b>Your pace and ticket</b><small>Choose daily, weekly, or monthly, then set the amount for each accepted card.</small></p>
          <p><span>2</span><b>Private personalization</b><small>Your cadence, ticket, risk mode, and asset mix become bounded 0G input.</small></p>
          <p><span>3</span><b>You approve execution</b><small>AI ranks; deterministic policy checks; your wallet signs.</small></p>
        </div>
      </section>
      ...
      <section className="onboarding-action">
        {isQuestionStep(step) ? (
          ...
          <>
            ...
            <button type="button" className="onboarding-text-button" onClick={changeAnswers}>
              Change my answers
            </button>
            <small>
              {config.demoMode
...
function QuestionFlow({
  ...
  step: Extract<Step, "welcome" | "cadence" | "ticket" | "risk" | "assets" | "review">;
  draft: PreferenceDraft;
  onDraft: React.Dispatch<React.SetStateAction<PreferenceDraft>>;
  onStep: (step: Step) => void;
  onSave: () => void;
}) {
  const questionNumber = ["cadence", "ticket", "risk", "assets", "review"].indexOf(step) + 1;
  ...
  if (step === "welcome") {
    return (
      <>
        ...
        <p>
          Choose how often you invest and the amount for each accepted card.
          The 100 USDG period limit stays fixed; unused funds remain in your wallet.
        </p>
        <div className="onboarding-trust-note">
          <Shield />
          <span><b>Non-custodial</b>Your answers never authorize a transaction.</span>
        </div>
        <button type="button" className="button button-primary" onClick={() => onStep("cadence")}>
    ...
    <>
      <div className="question-progress">
        ...
        <div aria-hidden="true">
          {[1, 2, 3, 4, 5].map((number) => (
            <i className={number <= questionNumber ? "active" : ""} key={number} />
          ))}
        </div>
      </div>
      ...
      {step === "cadence" ? (
        <>
          ...
          <p>Each period gets a separate session and a 100 USDG spending limit.</p>
          <div className="question-options">
            {CADENCE_OPTIONS.map((option) => (
              <button
                type="button"
                className={draft.cadence === option.id ? "question-option selected" : "question-option"}
                onClick={() => onDraft((current) => ({ ...current, cadence: option.id }))}
                key={option.id}
              >
        ...
        <>
          ...
          <div className="question-options ticket-options">
            {TICKET_OPTIONS.map((option) => (
              <button
                type="button"
                className={draft.ticketChoice === option.id ? "question-option selected" : "question-option"}
                onClick={() =>
                  onDraft((current) => ({
                    ...current,
                    ticketChoice: option.id,
                    ticketSizeUsd: typeof option.id === "number" ? option.id : customTicket(current.customTicketInput)
                  }))
                }
                key={option.id}
              >
                <span><b>{option.title}</b>{option.id === 10 ? <em>Recommended</em> : null}</span>
                <small>{option.description}</small>
        ...
        <>
          ...
          <div className="question-options">
            {RISK_OPTIONS.map((option) => (
              <button
                type="button"
                className={draft.riskMode === option.id ? "question-option selected" : "question-option"}
                onClick={() => onDraft((current) => ({ ...current, riskMode: option.id }))}
                key={option.id}
              >
                <span><b>{option.title}</b>{option.tag ? <em>{option.tag}</em> : null}</span>
                <small>{option.description}</small>
      ...
      {step === "assets" ? (
        <>
          ...
          <p>Tokenized stocks appear only after eligibility and market checks pass.</p>
          <div className="question-options">
            {ASSET_OPTIONS.map((option) => (
              <button
                type="button"
                className={draft.assetChoice === option.id ? "question-option selected" : "question-option"}
                onClick={() =>
                  onDraft((current) => ({
                    ...current,
      ...
      {step === "review" ? (
        <>
          ...
          <label className="risk-acknowledgement">
            ...
            />
            <span>
              I understand AI provides a ranking, not financial advice; assets can lose value;
              tokenized stocks depend on eligibility; and every trade requires my wallet approval.
            </span>
          </label>
          <QuestionActions
            back={() => onStep("assets")}
...
function QuestionActions({
  ...
  return (
    <div className="question-actions">
      ...
      <button type="button" className="button button-primary" onClick={next} disabled={nextDisabled}>
...
function PlanSummary({
  ...
  const risk = RISK_OPTIONS.find((option) => option.id === preferences.riskMode)?.title;
  const assets =
  ...
  return (
    <div className={compact ? "plan-summary compact" : "plan-summary"}>
      <p><span>Frequency</span><b>{cadenceLabel(preferences.cadence)}</b></p>
      <p><span>Ticket size</span><b>{preferences.ticketSizeUsd} USDG per card</b></p>
      <p><span>Period limit</span><b>100 USDG · up to {maxCardsFor(preferences.ticketSizeUsd)} cards</b></p>
      <p><span>Risk mode</span><b>{risk}</b></p>
      <p><span>Asset mix</span><b>{assets}</b></p>
    </div>
  );
}
...
function isQuestionStep(
  step: Step
): step is Extract<Step, "welcome" | "cadence" | "ticket" | "risk" | "assets" | "review"> {
  return ["welcome", "cadence", "ticket", "risk", "assets", "review"].includes(step);
}

function assetClassesFrom(choice?: AssetChoice): OnboardingPreferences["assetClasses"] {
...
function readStoredPreferences(): OnboardingPreferences | undefined {
  try {
    ...
    if (
      ...
      preferences.assetClasses.some(
        (value) => value !== "CRYPTO" && value !== "STOCK_TOKEN"
      ) ||
      preferences.riskDisclosureAccepted !== true
    ) {
      return;
    }
    return preferences as OnboardingPreferences;
  } catch {
    return;
  }
}
